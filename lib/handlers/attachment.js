 ...
 * @param {object} contextRouter - Context Router Instance
 * @returns {function} - Returns an attachmentHandler
 */
function attachmentHandlerFactory(locationHandler, yes_noAnswer = defaultYes_noAnswer, messages = {}, fb, contextRouter) {
    ...
    return (id, attachment, user) => startsTyping(id).then(() => {
        ...
        if (attachment.payload) {
            ...
            if (attachment.payload.url) {
                return contextRouter.getContextRoute(id, user, attachment.payload.type, attachment.payload.url);
            }
        }

        return attachmentDefault(id, user, attachment);
    });
}
